...
The change is hot-loaded: a JCR edit to `brxdis:environment` triggers cache invalidation; the new base URIs are picked up on the next request without a server restart.
...
- On the site side, `DiscoveryConfigProvider` is registered in `HippoServiceRegistry` so the CMS daemon module can reach it across the classloader boundary.
