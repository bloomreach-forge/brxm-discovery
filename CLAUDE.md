...
## Project Overview
A brXM PaaS plugin that integrates Bloomreach Discovery with brXM. Discovery is the search/recommendations brain; brXM is the content and page composition layer. The plugin provides HTTP/2-backed services and native HST components for search, category browse, and recommendations. Product catalog sync is handled externally by commerce connectors - this plugin is read-only against Discovery.
...
├── hcm-site/                (brxm-discovery-hcm-site - HCM bootstrap: node types, catalog, FTL templates)
└── site/                    (brxm-discovery-site - domain model, services, HTTP transport, HST components)
```

## Tech Stack
- **Java 17** (LTS) - use records, sealed classes, switch expressions, `.toList()`
- **brXM / Hippo CMS 17.0.0** - parent POM: `hippo-cms7-project:17.0.0`
...
├── site
│   ├── service/discovery/           # DiscoveryApiClient (interface), DiscoveryClientImpl (thin coordinator),
│   │                                #   ClientContextExtractor, ClientContext, SoREnrichmentProvider,
│   │                                #   DiscoveryPixelTransport, DefaultDiscoveryPixelTransport,
│   │                                #   DiscoveryRequestHeaders, DiscoveryRequestLogging
│   ├── service/discovery/dto/       # Shared raw shapes (4 files): ApiResponseBody, ProductDoc,
│   │                                #   ProductDocMapper (static toProductSummary/toVariantSummary),
│   │                                #   VariantDoc
│   ├── service/discovery/search/    # SearchApiClient (interface), SearchApiClientImpl,
│   │                                #   SearchResponseMapper, QueryParamParser
│   ├── service/discovery/search/dto/  # CampaignDto, FacetCounts, FacetFieldDto, FacetValueDto,
│   │                                  #   FieldStatsEntryDto, KeywordRedirectDto, SearchApiResponse, StatsDto
│   ├── service/discovery/search/model/ # CategoryHighlight, CategoryQuery, SearchResult, Facet,
│   │                                   #   FacetValue, PaginationModel, ProductSummary,
│   │                                   #   AutosuggestQuery, AutosuggestResult
│   ├── service/discovery/autosuggest/  # AutosuggestApiClient (interface), AutosuggestApiClientImpl,
│   │                                   #   AutosuggestResponseMapper
│   ├── service/discovery/autosuggest/dto/ # AutosuggestResponse
│   ├── service/discovery/recommendation/  # RecommendationApiClient (interface), RecommendationApiClientImpl,
│   │                                      #   RecommendationResponseMapper
│   ├── service/discovery/recommendation/dto/   # RecommendationResponse
│   ├── service/discovery/recommendation/model/ # RecQuery, DiscoveryRecommendationConfig,
│   │                                           #   RecommendationResult
│   ├── service/discovery/pixel/     # DiscoveryPixelService (interface), DiscoveryPixelServiceImpl,
│   │                                #   DeferredPixelInteractionHandler, PixelFlagsResolver,
│   │                                #   PixelFlags, PixelUserAgentPolicy, DeferredPixelEvent
│   ├── service/discovery/pixel/event/ # PixelEvent (sealed), SearchSubmit, SuggestClick, WidgetClick,
│   │                                  #   SearchPageView, ProductPageView, CategoryPageView, etc.
...
│   ├── platform/                    # HstDiscoveryService (facade), DiscoveryRequestCache,
│   │                                #   DiscoveryRuntimeContext, DiscoveryRuntimeContextFactory,
│   │                                #   DiscoveryBrUid2Service, DiscoveryBrUid2, CategoryPreviewCache
│   ├── component/                   # SearchRequestOptions, AbstractDiscoveryComponent + all HST components:
...
- **All HST/CMS deps are `provided` scope** - plugin is a library, host project supplies runtime
...
- **HTTP transport**: `DiscoveryHttpClientFactory.create()` builds a `java.net.http.HttpClient` with `Executors.newVirtualThreadPerTaskExecutor()`, HTTP/2, and a 10 s connect timeout. `HttpClientDiscoveryTransport` wraps it; sub-clients (`SearchApiClientImpl`, `AutosuggestApiClientImpl`, `RecommendationApiClientImpl`) take it as a constructor arg
...
- **Transport lookup**: `brxmdis.transport` (`HttpClientDiscoveryTransport`) is a Spring singleton; injected into sub-clients via constructor arg — never looked up from a registry
...
| Dependency | Scope |
|---|---|
| HST (api/commons/client), Repository API, CMS API, JCR, Jackson, SLF4J, spring-beans | provided |
