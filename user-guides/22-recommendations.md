...
Four separate HST components call the Discovery Recommendations API and each exposes a `products` list for your template:
...
| `DiscoveryKeywordRecommendationComponent` | Recommendations driven by a search keyword (e.g. "trending for query") |
...
| Discovery Keyword Recommendation | `brxdis:keywordRecommendationDocument` | `discoveryKeywordRecommendationWizard` | keyword/query-based widgets |
  ...
  config:
    /hst:hst/hst:configurations/<your-site>/hst:components:
      ...
      /keyword-recommendations:
        jcr:primaryType: hst:component
        hst:componentclassname: org.bloomreach.forge.discovery.site.component.DiscoveryKeywordRecommendationComponent
        hst:template: brxdis-recommendations-keyword
```
...
Set in HST config via `@ParametersInfo` (visible in the Channel Manager component editor). All four recommendation components share the same base parameters; only the URL-param name differs.
...
### `DiscoveryKeywordRecommendationComponent` only

The keyword component resolves the query in two modes (configured via the recommendation wizard):

- **specific** — uses the fixed keyword authored in the recommendation document
- **url** — reads the `?q=` request parameter at runtime (falls back to empty)

| Parameter | Group | Type | Default | Description |
|---|---|---|---|---|
| `document` | Recommendations | JCR path | - | Picker for a `brxdis:keywordRecommendationDocument` (stores widget config + keyword/mode as JSON). |
| `limit` | Recommendations | int | `8` | Default number of recommendations. |
| `showPrice` | Recommendations | boolean | `true` | Whether the template shows price. |
| `showDescription` | Recommendations | boolean | `false` | Whether the template shows description. |
