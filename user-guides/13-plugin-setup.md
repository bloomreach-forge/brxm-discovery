...
This guide picks up where installation ends. After adding the Maven dependencies and confirming the plugin bootstrapped, walk through the steps below to configure the plugin for your project.

> **Already here?** If you haven't installed yet, start at [02-installation.md](02-installation.md).
...
The plugin substitutes `staging-core.dxpapi.com`, `staging-pathways.dxpapi.com`, and `staging-suggest.dxpapi.com` automatically.

See [12-credential-injection.md](12-credential-injection.md) for more deployment patterns.
...
| Single account, multiple catalog views (region / locale / currency) | `view_id` via `withViewId()` — see below |
...
### Catalog views (`view_id`)

Some Discovery accounts are configured with multiple **catalog views** — named slices of the same account that serve different price lists, locales, or regional catalogs (distinct from separate accounts). When that feature is enabled, the Discovery API accepts a `view_id` parameter to select the active view.

`SearchQuery`, `CategoryQuery`, and `RecQuery` all support this via a `withViewId(String)` wither:

```java
// In a custom query factory or HstDiscoveryService subclass:
SearchQuery query = queryParamParser.toSearchQuery(...)
        .withViewId(resolveViewId(request));   // e.g. "en_US", "fr_CA", "de_DE"
```

`view_id` is not exposed as a component parameter (it is account-level infrastructure, not editorial config). The typical integration pattern is to resolve it from the channel config, a sitemap parameter, or a URL prefix, then pass it through a custom extension of `HstDiscoveryService` or `QueryParamParser`.
...
|---|---|
| [20-search-and-category.md](20-search-and-category.md) | Component parameters, URL params, facet and pagination wiring |
| [22-recommendations.md](22-recommendations.md) | Recommendation widgets, v2 Pathways API setup |
| [30-product-picker.md](30-product-picker.md) | CMS product/category picker for document types |
| [12-credential-injection.md](12-credential-injection.md) | Credential precedence, multi-environment patterns |
| [40-spa-integration.md](40-spa-integration.md) | Page Model API shapes, TypeScript types, React component examples |
| [50-pixel-tracking.md](50-pixel-tracking.md) | Pixel event configuration and per-channel flags |
