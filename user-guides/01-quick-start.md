...
This guide walks through every step from an empty brXM 17.0.0 project to a working search results page in the browser.
...
For production, use environment variables (`BRXDIS_ACCOUNT_ID`, `BRXDIS_DOMAIN_KEY`, `BRXDIS_API_KEY`) or the global Discovery config node. If different channels need different account/domain values or env-var names for secrets, use the channel-level `DiscoveryChannelInfo` fields in `hst:channelinfo` - see [12-credential-injection.md](12-credential-injection.md) for deployment patterns.

---

## Step 3 - Create the Discovery config node (optional)
...
> For category browse pages use `DiscoveryCategoryGridComponent` with the same template. See [20-search-and-category.md](20-search-and-category.md) for both.
...
| Guide | Topic |
|---|---|
| [10-discovery-config.md](10-discovery-config.md) | Config document fields and defaults |
| [20-search-and-category.md](20-search-and-category.md) | Full parameter reference for search and category components |
| [22-recommendations.md](22-recommendations.md) | Recommendation widgets, v2 Pathways API |
| [12-credential-injection.md](12-credential-injection.md) | Credential precedence and deployment patterns |
| [21-search-input-autosuggest.md](21-search-input-autosuggest.md) | Autosuggest dropdown, suggest-only mode |
