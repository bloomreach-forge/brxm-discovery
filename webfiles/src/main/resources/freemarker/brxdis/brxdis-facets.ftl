...
<@hst.defineObjects/>
<@hst.headContribution category="css" keyHint="brxdis-facets-css">
    <style>
        nav.brxdis-facets {
        ...
        .brxdis-facets__chip:hover {
            background: #fee2e2;
            border-color: #fca5a5;
            color: #991b1b
        }

        .brxdis-facets__chip-x {
            font-size: .875rem;
            line-height: 1;
            opacity: .7;
            margin-top: -1px
        ...
        .brxdis-facets__chip-x:hover {
            opacity: 1
        }

        .brxdis-facets__clear {
            margin-left: .25rem;
            font-size: .75rem;
            color: #6b7280;
            text-decoration: none;
        ...
        .brxdis-facets__group:last-child {
            border-bottom: 1px solid #e5e7eb
        }

        .brxdis-facets__group details {
            padding: 0
        }

        .brxdis-facets__group summary {
            display: flex;
        ...
        .brxdis-facets__group summary::-webkit-details-marker {
            display: none
        }

        .brxdis-facets__group-name {
            font-size: .6875rem;
            font-weight: 700;
        ...
        .brxdis-facets__group details[open] .brxdis-facets__chevron {
            transform: rotate(180deg)
        }

        .brxdis-facets__group-body {
        ...
        .brxdis-facets__item a:hover {
            background: #f9fafb
        }

        .brxdis-facets__item a:focus-visible {
            outline: 2px solid #2563eb
        }

        /* Checkbox indicator */
        .brxdis-facets__check {
            width: 14px;
            height: 14px;
        ...
        .brxdis-facets__item--active .brxdis-facets__check {
            background-color: #2563eb;
            border-color: #2563eb;
            background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 10 8' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M1 4l3 3 5-6' stroke='%23fff' stroke-width='1.8' fill='none' stroke-linecap='round'/%3E%3C/svg%3E")
        }

        /* Value label + count */
        .brxdis-facets__label {
            flex: 1;
            min-width: 0;
            overflow: hidden;
        ...
        .brxdis-facets__item--active .brxdis-facets__label {
            font-weight: 600;
            color: #1e40af
        }

        .brxdis-facets__count {
            font-size: .6875rem;
            color: #9ca3af;
            background: #f9fafb;
            border-radius: 999px;
        ...
        .brxdis-facets__item--active .brxdis-facets__count {
            background: #eff6ff;
            color: #1e40af
        }
            ...
            <div class="brxdis-facets__chips">
                <#list facets?values as facet>
                    ...
                    <#list activeVals as av>
                        <a href="${buildUrl("","",fp,av)}" class="brxdis-facets__chip"
                           title="Remove filter: ${facet.name()} = ${av}">
                            <span>${facet.name()}: ${av}</span>
                            <span class="brxdis-facets__chip-x" aria-hidden="true">&#x2715;</span>
                        </a>
                    </#list>
                </#list>
                <a href="${buildClearAllUrl()}" class="brxdis-facets__clear" title="Remove all active filters">Clear
                    all</a>
            </div>
        </#if>
            ...
            <div class="brxdis-facets__group">
                <details open>
                    <summary>
                        ...
                        <svg class="brxdis-facets__chevron" width="12" height="12" viewBox="0 0 12 12" fill="none"
                        ...
                        <ul class="brxdis-facets__list" role="group" aria-label="${facet.name()} filters">
                            <#list facet.value() as fv>
                                ...
                                <li class="brxdis-facets__item<#if isActive> brxdis-facets__item--active</#if>">
                                    <#if isActive>
                                    <a href="${buildUrl("","",fp,fv.name())}" role="checkbox" aria-checked="true"
                                       title="Remove ${facet.name()}: ${fv.name()}">
                                        <#else>
                                        <a href="${buildUrl(fp,fv.name(),"","")}" role="checkbox" aria-checked="false"
                                           title="Filter by ${facet.name()}: ${fv.name()}">
                                            </#if>
...
<#elseif editMode?? && editMode>
    <#assign _label = label!"default">
    <#if labelConnected?? && labelConnected>
        <div class="brxdis-empty">&#128270; <strong>Facets</strong> &mdash; connected to label
            <strong>${_label}</strong>.<br>Filters appear here once a search or category query is active.
        </div>
    <#else>
        <div class="brxdis-empty">&#128736; <strong>Facets</strong> &mdash; no data source connected.<br>Add a Discovery
            Search or Category component with a matching label.
        </div>
    </#if>
</#if>
